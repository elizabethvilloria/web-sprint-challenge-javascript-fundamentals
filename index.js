...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
// the reason why nested function scan access the variable internal is because of function scopes. The rule is one-way street where functions can reach out to get variables outside of their function but cannot reach in.
...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(arr){
    let displayNames = [];
    arr.forEach((animals) => {
      let display = `name: ${animals.animal_name}, scientific: ${animals.scientific_name}`;
      displayNames.push(display);
    });
    return displayNames;
  }
console.log('Task req 1:',animalNames(zooAnimals));  
  ...
  */
  ...
  function lowerCaseNames(arr){
    ...
    for(let i = 0; i < arr.length; i++) {
      let names = [];
      names = arr[i].animal_name;
      animalNames.push(names);
    }
    
    var lowerCase = animalNames.map(x => x.toLowerCase());
    return lowerCase;
  }

  console.log('Req 1:', lowerCaseNames(zooAnimals));
  ...
  */
  ...
  function lowPopulationAnimals(arr){
    const lowPop = arr.filter((animal) => {
  ...
  */
  ...
  function USApop(arr){
    const totalPop = arr.reduce((total, animals) => {
